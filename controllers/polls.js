...
export const addStudentResponse = async (request, response) => {
  try {
      console.log(request.body);
    let resp = await pollResponse.find({
      poll: new mongoose.Types.ObjectId(request.params.pollId),
    });
    resp = resp[0];
    for (let n = 0; n < request.body.answers.length; n++) {
      ...
      resp[request.body.answers[n].type][index].responses.push(
        request.body.answers[n].response
      );
    }
...
export const getStudentPolls = async (request, response) => {
  let groupMap = {
    ...
    'Faculty Sponsor UG Freshman Batch-SEECS': {
      batch: '2021',
      graduateLevel: 'UG',
    },
    'Faculty Sponsor UG Sophomore Batch-SEECS': {
      batch: '2020',
      graduateLevel: 'UG',
    },
    'Faculty Sponsor UG Junior Batch-SEECS': {
      batch: '2019',
      graduateLevel: 'UG',
    },
    'Faculty Sponsor UG Senior Batch-SEECS': {
      batch: '2018',
      graduateLevel: 'UG',
    },
    'Faculty Sponsor PG 1st Year Batch-SEECS': {
      batch: '2020',
      graduateLevel: 'MS',
    },
    'Faculty Sponsor PG 2nd Year Batch-SEECS': {
      batch: '2021',
      graduateLevel: 'MS',
    },
